<script lang="ts">
	...
	async function saveSystemPrompt() {
		...
		try {
			await graphql(UPDATE_SESSION_PROMPT, {
				id: activeSessionId,
				prompt: systemPrompt,
			});
...
<div
	class="flex h-screen w-full bg-slate-50 dark:bg-zinc-950 text-slate-900 dark:text-slate-100 overflow-hidden font-sans relative"
>
	<!-- Mobile Overlay -->
	{#if isSidebarOpen}
	...
	<!-- Main Chat Area -->
	<main
		class="flex-1 flex flex-col bg-slate-50/50 dark:bg-zinc-950 min-w-0 relative"
	>
		<!-- Header -->
		<header
			class="h-16 border-b border-slate-200 dark:border-zinc-800 bg-white/50 dark:bg-zinc-900/30 backdrop-blur-md shrink-0 flex items-center px-4 md:px-8 relative z-10 w-full"
		>
			<button
				class="md:hidden mr-4 p-2 -ml-2 text-slate-600 hover:bg-slate-100 dark:hover:bg-zinc-800 rounded-md transition-colors"
				on:click={toggleSidebar}
			>
				<Menu class="w-5 h-5" />
			</button>
			...
			<div class="max-w-3xl mx-auto space-y-6 md:space-y-8 flex flex-col">
				...
				{#if isTyping}
					<div class="flex justify-start opacity-70">
						<div
							class="px-5 py-3.5 rounded-2xl bg-white dark:bg-zinc-900 border border-slate-100 dark:border-zinc-800 rounded-tl-sm text-slate-500 italic text-sm shadow-sm flex items-center"
						>
